...
select dv.id_dich_vu,dv.ten_dich_vu,dv.dien_tich,dv.so_nguoi_to_da,dv.chi_phi_thue,ldv.ten_loai_dich_Vu
from dich_vu dv
inner join loai_dich_vu ldv on ldv.id_loai_dich_vu=dv.id_loai_dich_vu
inner join hop_dong hd on hd.id_dich_vu = dv.id_dich_vu
where (year(hd.ngay_lam_hop_dong)=2018)
and (year (hd.ngay_ket_thuc)=2018)
and hd.id_hop_dong not in (year(hd.ngay_lam_hop_dong)=2019);
...
select hd.id_hop_dong,dv.ten_dich_vu,hd.ngay_lam_hop_dong,count(month(ngay_lam_hop_dong))'tong_so_khach_dat_hang_trong_thang'
-- (dv.chi_phi_thue+hdct.so_luong*dvdk.gia*'tong_so_khach_dat_hang_trong_thang')as 'tong_thue_thang'
from hop_dong hd
inner join dich_vu dv on dv.id_dich_vu=hd.id_dich_vu
-- inner join hop_dong_chi_tient hdct on hdct.id_hop_dong=hd.id_hop_dong
...
select hd.id_hop_dong,hd.ngay_lam_hop_dong,hd.ngay_ket_thuc,hd.tien_dat_coc,hdct.so_luong,
count(hdct.id_hop_dong_chi_tiet)'tong'
from hop_dong hd
inner join hop_dong_chi_tient hdct on hd.id_hop_dong=hdct.id_hop_dong
...
inner join hop_dong_chi_tient hdct on hdct.id_dich_vu_di_kem=dvdk.id_dich_vu_di_kem
...
select hd.id_hop_dong,nv.ho_ten as 'ho_ten_nhan_vien',kh.ho_ten as 'ho_ten_khach_hang',kh.sdt,
dv.ten_dich_vu,hdct.so_luong,hd.tien_dat_coc
from hop_dong hd
...
(select hd.tien_dat_coc
from hop_dong
where (year(ngay_lam_hop_dong)=2019) and (month(ngay_lam_hop_dong)in(1,2,3,4,5,6))
);
